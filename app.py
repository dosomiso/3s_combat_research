...
# 預設密碼（你可以修改）
import os
PASSWORD = os.getenv("SYSTEM_PASS", "91367**")  # 預設密碼為 91367**
...
def wrest_overview():
    ...
    # 將表格和文件名傳遞給模板
    return render_template('wrest_display.html', tables=tables)

# 顯示所有.xlsx檔案表格 (柔道前台)
@app.route('/judo_display')
def judo_overview():
    # 前台頁面
    if not session.get('authenticated'):
        return redirect(url_for('login'))
    ...
    # 將表格和文件名傳遞給模板
    return render_template('judo_display.html', tables=tables)

# 顯示所有.xlsx檔案表格 (克拉柔前台)
@app.route('/kyc_display')
def kyc_overview():
    # 前台頁面
    if not session.get('authenticated'):
        return redirect(url_for('login'))
    ...
    # 將表格和文件名傳遞給模板
    return render_template('kyc_display.html', tables=tables)
